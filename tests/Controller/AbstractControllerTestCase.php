...
abstract class AbstractControllerTestCase extends TestCase
{
    ...
    protected function assertNotFoundResponse(ResponseInterface $response, string $resourceName, string $id)
    {
        ...
        $this->assertEquals(sprintf('%s with id "%s" not found', $resourceName, $id), $data['detail']);
        $this->assertEquals('requestUri', $data['instance']);
    }
    ...
    protected function assertJsonResult(Result $result, array $data)
    {
        ...
        $this->assertEquals($data['restarts'], $result->getRestarts());
